...
const appStorage = {
    ...
    setItem: function(key, value) {
        try {
            ...
            console.error("appStorage write error", e);
        }
    },
    removeItem: function(key) {
        try {
            if (fs.existsSync(storageFilePath)) {
                try {
                    let data = JSON.parse(fs.readFileSync(storageFilePath, 'utf8'));
                    delete data[key];
                    fs.writeFileSync(storageFilePath, JSON.stringify(data));
                } catch(err) {}
            }
...
function showProductContextMenu(e, product) {
    ...
    // Image options based on whether image exists
    if (product.image) {
    ...
    if (process.env.PORTABLE_EXECUTABLE_DIR) {
        ...
        try {
            ...
            if (electronApp) {
                ...
                baseDir = path.join(require('os').homedir(), 'AppData', 'Roaming', 'duranlux-pos');
            }
        } catch(e) {
            baseDir = path.join(require('os').homedir(), 'AppData', 'Roaming', 'duranlux-pos');
        }
